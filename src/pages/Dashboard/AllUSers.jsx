...
const AllUsers = () => {
  const { refetch, data: users = [] } = useQuery({
    queryKey: ['register'],
    queryFn: async () => {
      const res = await axiosSecure.get('http://localhost:3000/register');
      return res.data;
    },
  });

  const handleMakeAdmin = (user) => {
    axiosSecure.patch(`/register/admin/${user._id}`)
      .then((res) => {
  ...
  const handleMakeUser = (user) => {
    axiosSecure.patch(`/register/user/${user._id}`)
      .then((res) => {
        if (res.data.modifiedCount > 0) {
          refetch();
  ...
  const handleMakeCreator = (user) => {
    axiosSecure.patch(`/register/creator/${user._id}`)
      .then((res) => {
        if (res.data.modifiedCount > 0) {
          refetch();
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`http://localhost:3000/register/${id}`)
          .then((res) => {
                ...
                <tr key={item._id}>
                  ...
                  <td>
                    {item.role === 'admin' ? 'Admin' : (
                      <button className="btn btn-ghost btn-xs" onClick={() => handleMakeAdmin(item)}>
                  ...
                  <td>
                    {item.role === 'user' ? (
                      <button className="btn btn-ghost btn-xs" onClick={() => handleMakeCreator(item)}>
                        Make Creator
                      </button>
                    ) : (
                      <button className="btn btn-ghost btn-xs" onClick={() => handleMakeUser(item)}>
                        Make User
                      </button>
                    )}
                  </td>
                  <th>
                    <button className="btn btn-ghost btn-xs" onClick={() => handleDelete(item._id)}>
